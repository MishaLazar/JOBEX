...
from mongo_db_handler import Client
...
class JobThread(threading.Thread):
    ...
    def do_job(self):
        ...
        for j in jobs:
            ...
            job = Job(job_id=json_obj['_id'], job_type_id=int(json_obj['job_type_id']), source_objectid=json_obj['source_objectid'],
                      creation_date=json_obj['creation_date'], status=int(json_obj['status']))
    ...
    def get_jobs(self):
        ...
        return db_client.get_many_docs_from_collection(DbCollections.get_collection('jobs'), query)
    ...
    def get_all_active_positions(self):
        ...
        return db_client.get_many_docs_from_collection(DbCollections.get_collection('positions_col'), query)
    ...
    def get_all_active_students(self):
        ...
        return db_client.get_many_docs_from_collection(DbCollections.get_collection('users_col'), query)

    def on_job_finish(self, job, status):
        db_client = Client()
        filter_json = {"_id": job.job_id}
